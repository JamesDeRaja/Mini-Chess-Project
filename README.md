...
It is not trying to replace standard chess.  
It is designed to make chess faster, less intimidating, more replayable, and easier to share.
...
```txt
Pocket Shuffle Chess is for fast daily tactical battles, not long opening-theory chess.
```
...
```txt
I won Pocket Shuffle Chess in 18 moves.
...
Keep the project simple and playable first.

Do not overbuild accounts, leaderboards, engine analysis, or complex chess rules before the core loop is fun.
...
5. Friend challenge
6. Mirror Match
7. Stats and leaderboards later
```

The winning version of this game is not the most complete chess engine.

The winning version is the one where a player thinks:

```txt
That was quick.
I want to try one more seed.
