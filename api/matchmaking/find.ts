...
async function getOpenGameId(supabase: ServerSupabase, gameId: string): Promise<string | null> {
...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  if (existing?.status === 'matched' && existing.game_id) {
    const openGameId = await getOpenGameId(supabase, existing.game_id);
    if (openGameId) {
      response.status(200).json({ status: 'matched', gameId: openGameId });
      return;
    }
    await supabase.from('matchmaking_queue').update({ status: 'expired' }).eq('id', existing.id);
  }
