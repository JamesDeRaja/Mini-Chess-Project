...
import { getBestMoveChanceForPower, getPlayerPowerTier } from '../game/playerPower.js';
...
function BotGameContent({ matchMode, dateKey: requestedDateKey, customSeed, customBackRankCode, playerSide, activeChallengeContext, onHome, seedValidation }: BotGameContentProps) {
  ...
  function startReviewVariation() {
    if (!roundResult) return;
    pauseHistoryAutoplay();
    const branchPly = activeReviewPly && activeReviewPly > 0 ? activeReviewPly - 1 : previewPly ?? latestPly;
    const branchBoard = boardTimeline[branchPly];
    if (!branchBoard) return;
    setVariationStartPly(branchPly);
    setVariationBoard(cloneBoard(branchBoard));
    setVariationTurn(branchPly % 2 === 0 ? 'white' : 'black');
  ...
  function selectSquare(squareIndex: number): boolean {
    const activeBoard = variationBoard ?? board;
    const activeTurn = variationTurn ?? turn;
    const canSelect = isVariationReview || (status === 'active' && activeTurn === playerColor && !isPreviewing);
  ...
  function tryMoveTo(squareIndex: number): boolean {
    const canMove = isVariationReview || (status === 'active' && turn === playerColor && !isPreviewing);
    if (!canMove) return false;
    const selectedMove = legalMoves.find((move) => move.to === squareIndex);
    if (selectedMove) {
      if (isVariationReview) completeVariationMove(selectedMove);
  ...
  function handleDragStart(squareIndex: number): Move[] | null {
    const activeBoard = variationBoard ?? board;
    const activeTurn = variationTurn ?? turn;
    const canDrag = isVariationReview || (status === 'active' && activeTurn === playerColor && !isPreviewing);
  ...
  function handleDrop(squareIndex: number, draggedMove?: Move) {
    if (draggedMove) {
      if (isVariationReview) completeVariationMove(draggedMove);
  ...
  const activeLegalMoves = (isVariationReview || (!isPreviewing && !roundResult)) && isBoardReady ? legalMoves : [];
  ...
  return (
    <main className="game-page">
      <GameHeader
        ...
        details={dailyAIDifficulty ? `Daily ladder · ${dailyAIDifficulty} bot · Power ${playerPowerTier * 10}% · ${dailyAIProgress.stars}${dailyAIProgress.magicStarUnlocked ? ' + magic' : ''} stars` : `${config.label} · ${botLevel} bot · Power ${playerPowerTier * 10}%`}
        onTitleClick={onHome}
      ...
      <div className="game-layout chess-shell">
        ...
        <section className="board-column">
          ...
          <Board
            ...
            isFlipped={isFlipped}
            isInteractive={isBoardReady && (isVariationReview || (!isPreviewing && status === 'active'))}
        ...
        <aside className="side-panel review-panel history-panel">
          <div className="history-header">
            ...
            <p className="panel-note">Click a move to review. Use Try move to branch, then Resume to return to the original replay.</p>
            {analysisPanel}
          </div>
          ...
          <div className="review-footer history-actions">
            <div className="review-controls">
              ...
              <button type="button" onClick={goToLiveReview}>{isVariationReview ? 'Resume' : 'Live'}</button>
            ...
            <div className={`panel-actions stacked-actions ${roundResult ? 'history-complete-actions' : ''}`}>
              {roundResult ? (
                <>
                  <button type="button" className="secondary-action" onClick={startReviewVariation} disabled={moveHistory.length === 0}>
                    <Sparkles size={18} /> Try alternate move
                  </button>
          ...
          actions={(
            <>
              <div className="result-action-row result-action-row-primary">
                <button type="button" className="result-primary-action" onClick={openShareModal}><Share2 size={17} /> Challenge a Friend</button>
                <button type="button" className="secondary-action result-primary-action" onClick={startPostGameReview}><Sparkles size={18} /> Review Game</button>
              </div>
              <div className="result-action-row result-action-row-tools">
                <ResultScreenshotButton title={roundResult.didPlayerWin ? 'You won' : roundResult.status === 'draw' ? 'Draw' : 'You lost'} summary={roundResult.message} score={scoreBreakdown.totalScore} moves={scoreBreakdown.fullMoves} seed={seedSlug} setup={dailySeedInfo.backRankCode} className="result-tool-action" />
                <button type="button" className="secondary-action result-tool-action" onClick={() => { void copyChallengeLink(); }}><Copy size={22} /> Copy Link</button>
              </div>
